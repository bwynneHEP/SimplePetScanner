...
def FindHitRadius( Event, DetectorRadius ):
  ...
  if deltaPhi < 0.0:
    return -DetectorRadius * math.cos( deltaPhi/2.0 ), Event
  else:
    return DetectorRadius * math.cos( deltaPhi/2.0 ), Event
...
def TwoHitEvent( Event, DetectorRadius, ZMin=0.0, ZMax=0.0, RMax=120.0 ):
  ...
  rMin, Event = FindHitRadius( Event, DetectorRadius )
...
def CreateDataset( DetectorLengthMM, Detector, SourceLengthMM, Source, TotalDecays, EnergyMin, EnergyMax, DetectorMaterial, SensitivityPhantom, nSleeves, Seed=1234, CoincidenceWindow=0.0 ):
  # Phantom length affects the attenuating material, so include it even if source is detector
  outputFileName = "hits.n" + str(TotalDecays) + "." + Detector + "Block." + str(DetectorLengthMM) + "mm." + "SensitivityPhantom." + str(nSleeves) + "sleeves."
  ...
  if os.path.exists( outputFileName ):
    ...
    command += " --detector " + Detector + "Crystal"
    command += " --detectorLengthMM " + str(DetectorLengthMM)
    command += " --source " + Source
    command += " --phantomLengthMM " + str(SourceLengthMM)
    command += " --SensitivityPhantom " + str(SensitivityPhantom)
    command += " --nSleeves " + str(nSleeves)
